...
All 22 contract checkpoints, eight dApp coding checks and two circuit checks run from static files. `hosting.js` selects browser mode outside loopback hosts, or with `?runtime=simulator`. Normal localhost use retains native execution with no silent fallback. Same-site assets are needed; no execution service, RPC, CDN, wallet or API key is used. Offline reopening is not implemented.
...
Browsing ahead preserves drafts/bookmarks without moving the active task past an unchecked exercise. Reviewing earlier chapters keeps the latest active ABI. Reading alone never awards coding skills; unearned recaps show expected results. Start with Dusk retains genuine knowledge gates. The original preview flags still permit browsing only, not fabricated checkpoints. Name edits preserve both histories across modes. Storage is origin-specific; copy drafts manually between Pages and localhost. Entry scripts and shared save/runtime modules use a versioned URL so a cached pre-browser snapshot writer cannot record simulated work as native credit. Worker assets are refreshed before execution without weakening native CSP checks. Bump those URL versions together when changing this contract, and reload open lesson tabs after an update.
...
The first lesson introduces Rust/Forge/DuskVM roles, the file structure, fields and constructor values, getter syntax and mutable entrypoints before the two original edits. Worked traces then separate persisted state from a fresh deployment, distinguish compiler/runtime/behavior failures and state what the counter does not prove. There are 28 worked-example guides and 19 optional practice questions across Contracts. Only the existing 22 execution checks award progress.
...
A submitted file uses the real `#[dusk_forge::contract]` macro. The server invokes pinned `rustc` with the prebuilt Forge/Dusk libraries, rather than rebuilding all dependencies with the CLI for every keystroke. WASM is then deployed and called by `dusk-vm` in a fresh isolated process. The first lesson uses parameterless `register()`. The second changes it to `register(amount)`, and the third adds the guards in the reference contract. The server accepts only fixed scenarios, with no client-supplied commands, native tests or arbitrary call sequences.
...
Atomic-pair tests force the *second* callee to fail after the first succeeds. Both contracts' writes must roll back. The test relay preserves raw receipt events, including attempted inner work visible in this pinned VM. Events are not treated as committed success or finality. Equivalent prevalidation/removal implementations pass. The final build checkpoint compiles both WASM targets and uses the genuine vendored SDK in an isolated Node checker for schema, argument encoding and exact largest-u64 decoding. Artifact bytes/hashes and method signatures are displayed. These raw tuple events are not registered Forge event types. A method schema does not supply an event schema.

The compiler and VM use the shared sandbox in `tools/forge_lesson.py`. Source, request, artifact, memory, gas and process limits remain enforced. Their values live with the implementation. The multi-contract VM alone uses a 32-GiB virtual address ceiling for Piecrust's per-instance reservations; compilation/driver checks remain at 12 GiB, and learner/fixture WASM remains capped at 16 MiB. A shared 40-second contract-request budget covers both builds, execution and SDK checking, in addition to per-process CPU/wall limits. These are local teaching runtimes, not independently reviewed public sandboxes or aggregate resource quotas. There is no unsandboxed fallback.
...
- **dApps (15 chapters):** app/node/wallet roles, contract IDs, async/await, real reads, integer precision, the ABI/data-driver, arguments versus token values, prepared calls, request review, discovery, approval, outcomes and recovery. One JavaScript file uses the real Connect SDK. Native reads reach actual DuskVM fixtures; browser reads use the simulated transport; the generated Forge driver encodes/decodes `register(u64)`. The two coding checkpoints and wallet question remain, with six optional practice questions. Prepared calls have zero attached tokens and are never submitted. Optional discovery/connection is a separate SDK flow without learner-code access to the injected wallet.
- **Circuits (12 chapters):** prover/verifier roles, witnesses, an addition gate, equality constraints, negative tests, Rust versus circuit checks, public inputs, verification, ranges and trusted-input boundaries. Two real coding checkpoints remain, with four optional practice questions. One Rust file uses `dusk-plonk 0.22.1`. Native mode compiles edited Rust; browser mode interprets its gates for a prebuilt engine. Both generate parameters and actual proofs, verifies valid sums and checks invalid witnesses and changed public inputs. See the [circuit harness](../examples/first-circuit/README.md).
...
The academy uses vanilla HTML/CSS/JavaScript and local artwork, fonts, SDK and WASM files. There are no analytics or browser keys. Native learner-JavaScript/compiled-circuit execution still requires the development server’s worker CSP headers. Browser dApps use the opaque-frame policy above instead; browser circuits execute only the trusted interpreter/prebuilt engine. Native header guards were not bypassed for arbitrary learner JavaScript.
...
| Read a record | Select `/api/explorer-driver`; add `readRegistration(dusk, id)` using actual `get_registration` reads |
...
The native local adapter allowlists just three getters with exactly eight argument bytes. Alias `55…55` runs a genuine raw call in a fresh isolated DuskVM; alias `66…66` deliberately responds with HTTP 503. The worker then explicitly retries the working alias. Original counter reads/prepared arguments are checked on every explorer run. Evidence includes actual HTTP status and encoded argument bytes for each lookup. Invalid IDs must be rejected before a request; constants without reads, rounded IDs, false-to-null conversion, fabricated missing records and swallowed failures fail the checks. These are finite learning checks, not tamper-proof certification.
...
`npm test`, `npm run test:forge`, `npm run test:circuits`, `tools/test_academy.cjs`, `tools/test_courses.cjs` and the backend-free `tools/test_pages.cjs` cover the implemented slice. The browser checks cover every expanded chapter, optional-practice feedback without fake checkpoints, compact/keyboard navigation, running the active code from worked examples, completed v1 migrations, actual calls/proofs, equivalent source continuity, independent saves, malformed/blocked storage, cancellation, missing workers and missing wallets. Real extension approval is not claimed by the no-wallet test.
