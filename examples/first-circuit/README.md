...
- `src/lib.rs` is a fixed host harness. Only the learner's circuit file is replaced
  in temporary compilation workspaces. HTTP never accepts Cargo files or runs Cargo.
...
The browser worker permits only two host functions: 32 bytes of cryptographic
entropy and one bounded JSON result (16 KiB). The fixed harness seeds a ChaCha20 RNG
